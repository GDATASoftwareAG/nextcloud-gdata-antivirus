...
class Provider implements IProvider
{
    ...
    public function parse($language, IEvent $event, ?IEvent $previousEvent = null)
    {
        ...
        if ($event->getSubject() === self::SUBJECT_VIRUS_DETECTED) {
            ...
            $parameters['virus'] = ['type' => 'highlight', 'id' => $params[1], 'name' => $params[1]];
            $parameters['file'] = ['type' => 'highlight', 'id' => $event->getObjectName(), 'name' => basename($event->getObjectName())];
            ...
            $params = $event->getSubjectParameters();
            $parameters['virus'] = ['type' => 'highlight', 'id' => $params[0], 'name' => $params[0]];
            ...
            $params = $event->getSubjectParameters();
            $parameters['virus'] = ['type' => 'highlight', 'id' => $params[0], 'name' => $params[0]];
            $parameters['file'] = ['type' => 'highlight', 'id' => $event->getObjectName(), 'name' => $event->getObjectName()];
            $event->setIcon($this->urlGenerator->imagePath('gdatavaas', 'favicon.svg'));
            if ($event->getMessage() === self::MESSAGE_FILE_DELETED) {
                $event->setParsedMessage('The file has been removed');
            }
    ...
    private function setSubjects(IEvent $event, string $subject, array $parameters): void
    {
        ...
        $event->setParsedSubject(str_replace($placeholders, $replacements, $subject))->setRichSubject($subject, $parameters);
    }
}
