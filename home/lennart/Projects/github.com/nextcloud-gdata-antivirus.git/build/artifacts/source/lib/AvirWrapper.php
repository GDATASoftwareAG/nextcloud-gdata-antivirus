...
use OCA\GDataVaas\Vendor\OC\Files\Storage\Wrapper\Wrapper;
...
use OCA\GDataVaas\Vendor\VaasSdk\Message\Verdict;
class AvirWrapper extends Wrapper
{
    ...
    {
        return $this->shouldScan && (!$this->isHomeStorage || (strpos($path, 'files/') === 0 || strpos($path, '/files/') === 0));
    }
    private function wrapSteam(string $path, $stream)
    {
        try {
            $logger = $this->logger;
            return \OCA\GDataVaas\CallbackReadDataWrapper::wrap($stream, null, null, function () use ($path, $logger) {
                ...
                if ($verdict->Verdict == Verdict::MALICIOUS) {
                    ...
                    if ($this->trashEnabled) {
                        ...
                        $trashManager = \OCA\GDataVaas\Vendor\OC::$server->query(ITrashManager::class);
                    ...
                    $this->logger->warning('Infected file deleted. ' . $verdict->Detection . ' Account: ' . $owner . ' Path: ' . $path, ['app' => 'gdatavaas']);
                    $activity = $this->activityManager->generateEvent();
                    $activity->setApp(Application::APP_ID)->setSubject(Provider::SUBJECT_VIRUS_DETECTED_UPLOAD, [$verdict->Detection ?? "no_detection_name"])->setMessage(Provider::MESSAGE_FILE_DELETED)->setObject('', 0, $path)->setAffectedUser($owner)->setType(Provider::TYPE_VIRUS_DETECTED);
                    ...
                    throw new InvalidContentException(sprintf('Virus %s is detected in the file. Upload cannot be completed.', $verdict->Detection));
                }
            });
        } catch (\Exception $e) {
    ...
    private function isWritingMode($mode)
    {
        ...
        $cleanMode = str_replace(['t', 'b'], ['', ''], $mode);
