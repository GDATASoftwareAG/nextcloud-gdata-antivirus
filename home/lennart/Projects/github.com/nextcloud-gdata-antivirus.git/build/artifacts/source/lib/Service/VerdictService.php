...
class VerdictService
{
    ...
    public function __construct(LoggerInterface $logger, IAppConfig $appConfig, \OCA\GDataVaas\Service\FileService $fileService, \OCA\GDataVaas\Service\TagService $tagService)
    {
        $this->logger = $logger;
    ...
    public function scanFileById(int $fileId): VaasVerdict
    {
        ...
        if (self::isFileTooLargeToScan($filePath)) {
            $this->tagService->setTag($fileId, \OCA\GDataVaas\Service\TagService::WONT_SCAN, silent: \true);
        ...
        $this->logger->info("VaaS scan result for " . $node->getName() . " (" . $fileId . "): Verdict: " . $verdict->Verdict->value . ", Detection: " . $verdict->Detection . ", SHA256: " . $verdict->Sha256 . ", FileType: " . $verdict->FileType . ", MimeType: " . $verdict->MimeType . ", UUID: " . $verdict->Guid);
    ...
    {
        switch ($tagName) {
            case \OCA\GDataVaas\Service\TagService::MALICIOUS:
                $this->tagService->setTag($fileId, \OCA\GDataVaas\Service\TagService::MALICIOUS, silent: \false);
                try {
                ...
                break;
            case \OCA\GDataVaas\Service\TagService::PUP:
                $this->tagService->setTag($fileId, \OCA\GDataVaas\Service\TagService::PUP, silent: \false);
                break;
            case \OCA\GDataVaas\Service\TagService::UNSCANNED:
                ...
                break;
            case \OCA\GDataVaas\Service\TagService::CLEAN:
            case \OCA\GDataVaas\Service\TagService::WONT_SCAN:
            default:
                $this->tagService->setTag($fileId, $tagName, silent: \true);
                break;
        }
    }
    /**
    ...
    public function tagLastScannedFile(string $localPath, int $fileId): void
    {
        if (self::isFileTooLargeToScan($localPath)) {
            $this->tagFile($fileId, \OCA\GDataVaas\Service\TagService::WONT_SCAN);
            return;
        }
        if (!$this->isAllowedToScan($localPath)) {
            $this->tagFile($fileId, \OCA\GDataVaas\Service\TagService::UNSCANNED);
            return;
        }
    ...
    public function getAuthenticator(string $authMethod): ClientCredentialsGrantAuthenticator|ResourceOwnerPasswordGrantAuthenticator
    {
        if ($authMethod === 'ResourceOwnerPassword') {
            return new ResourceOwnerPasswordGrantAuthenticator("nextcloud-customer", $this->username, $this->password, $this->tokenEndpoint);
        } elseif ($authMethod === 'ClientCredentials') {
            return new ClientCredentialsGrantAuthenticator($this->clientId, $this->clientSecret, $this->tokenEndpoint);
        } else {
