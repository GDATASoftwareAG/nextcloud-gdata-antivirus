...
setup_smtp () {
  ...
  sleep 1
  docker run --network nextcloud -d --name smtp-server --rm -it -p 80:80 -p 2525:25 rnwood/smtp4dev:v3

  until curl --fail localhost
  do
    echo "waiting for smtp to be initialized"
    sleep 2
  done
...
setup_nextcloud () {
  ...
  sleep 1
  docker run --network nextcloud --quiet -d --name nextcloud-container --rm --publish 8080:80 nextcloud:$NEXTCLOUD_VERSION
